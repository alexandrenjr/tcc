...
\begin{algorithm}[ht!]
  ...
  \begin{algorithmic}[1]
    ...
    \State $F \gets F \cap \eqref{eq:LMIESetorConicoEsquerdo}$ com $\alpha = V_o$ e $\theta = ang(Z,V_i)$ \Comment{Setor cônico esquerdo}
    \State $F \gets F \cap \eqref{eq:LMIESetorConicoDireito}$ com $\alpha = V_i$ e $\theta = ang(Z,V_o)$ \Comment{Setor cônico direito}
...
% \begin{align*}
%   \real{(\z)} = \exp{\left(-\zeta T_s\dfrac{\pi}{T_s\sqrt{1-\zeta^2}} + \jmath \pi\right)} = -\exp{\left(-\dfrac{\zeta}{\sqrt{1-\zeta^2}}\pi\right)}\label{eq:RealWN}
% \end{align*}
