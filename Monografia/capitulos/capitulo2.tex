...
A alocação de polos é uma das principais ferramentas da teoria de controle, pois a partir desta, é possível projetar um sistema que seja estável e que tenha um bom desempenho \citeonline{ROSINOVA2014}. A operação de alocar polos de um sistema linear dentro de uma região específica é chamada $\mathscr{D}$-estabilidade \citeonline{CHILALI1996}.
...
\begin{figure}[!hb]
  ...
  \end{subfigure}
  \caption{Regiões de $\mathscr{D}$-estabilidade do plano $\s$. Em \subref{subfig:EstabilidadeRelativaS} encontram-se retas verticais para vários valores de $\sigma$, sendo $|\sigma_2| > |\sigma_1|$. Em \subref{subfig:TaxaDeAmortecimentoS} encontram-se retas para vários valores de $\zeta$, sendo $\zeta_2 > \zeta_1$. Em \subref{subfig:OscilacaoNaoAmortecidaZ} encontram-se circunferências de raios $r = \omega_n$, sendo $\omega_{n2} > \omega_{n1}$.}
